...
const BASE_SUPPLIES: Record<
  "防灾" | "防疫" | "日常应急",
  Omit<SupplyItem, "quantityText" | "reason">[]
> = {
  防灾: [
    {
      id: "water",
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    // 验证输入
    if (!input.city || !input.people || !input.environment || !input.days) {
      return new Response(
    ...
    const lists: GeneratorOutput["lists"] = {
      防灾: [],
      防疫: [],
      日常应急: [],
    };

    for (const category of Object.keys(BASE_SUPPLIES) as Array<
      "防灾" | "防疫" | "日常应急"
    >) {
